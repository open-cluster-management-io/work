...
func TestDeleteAppliedResourcess(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			actual, err := DeleteAppliedResources(
				context.TODO(), c.resourcesToRemove, "testing", fakeDynamicClient, NewWorkResourceCache(), eventstesting.NewTestingEventRecorder(t), c.owner)
			if err != nil {
				t.Errorf("unexpected err: %v", err)
			}
